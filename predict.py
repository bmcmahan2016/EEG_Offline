...
def TrainNN(net, train_loader, criterion):
...
def TestNN(net, test_loader, criterion):
    net.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
...
def ClassifyNN(training_data, training_classes, num_classes=4):
    ...
    TrainNN(net, train_loader, criterion)
    TestNN(net, test_loader, criterion)
...
def main():
    ...
    data_manager = DataManager(bin_size=10, lowcut=30.0, highcut=60.0) # set parameters for data filtering and collection
    ...
    GetClassFrequency(training_classes, class_map) # print the class frequency in the data set
    ...
    # ClassifyNN(training_data, training_classes, num_classes=len(include_classes)) # train and evaluate neural network on this data set
